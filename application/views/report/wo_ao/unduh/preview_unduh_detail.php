<div class="box-body table-responsive">
    <h4>Monitoring Kelolaan WO AO PPK</h4>
    ...
    <tbody>
      <?php if (!empty($report_detail->result_array()) ): ?>
        ...
        <?php $no=1; foreach ($report_detail->result_array() as $r): 
          if ($r['kolektibilitas']) {
            $kol = $r['kolektibilitas'];
          } else {
            $kol = "Belum Teridentifikasi";
          }
          ?>
          <tr>
            <td><?= $no++ ?></td>
            ...
            <td><?= $r['deal_reff'] ?></td>
            <td><?= $r['nama'] ?></td>
            <td align="right"><?= number_format($r['tunggakan_pokok'],'0','.','.') ?></td>
            <td><?= ($r['id_kol'] == 12) ? 'Potensial' : "Parking - ".$kol ?></td>
            <td><?= $r['name'] ?></td>
          </tr>
        <?php endforeach ?>
