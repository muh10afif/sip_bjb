...
$conn = sqlsrv_connect( $serverName, $connectionInfo);
    ...
    $sql_del = "DROP TABLE report_npl_kanwil1_cabang";
    ...
    while( $row = sqlsrv_fetch_array( $hasil_seg, SQLSRV_FETCH_ASSOC) ) {
        ...
        if ($i == 1) {
            ...
            $sql_c = "CREATE TABLE report_npl_kanwil1_cabang (id INT IDENTITY PRIMARY KEY, nama VARCHAR(255) , cabang_induk VARCHAR(200), reg_employee VARCHAR(100) , urutan INT , $segmen INT,  add_time datetime DEFAULT GETDATE() )";

            $hasil_c = sqlsrv_query( $conn, $sql_c);
            
        } else {

            $sql_c = "ALTER TABLE report_npl_kanwil1_cabang ADD $segmen INT";
    ...
    $sql0 = "SELECT DISTINCT cabang_induk FROM m_debitur ORDER BY cabang_induk ASC";
    ...
    while( $row = sqlsrv_fetch_array( $hasil0, SQLSRV_FETCH_ASSOC) ) {
        ...
        while( $row_seg = sqlsrv_fetch_array( $hasil_seg, SQLSRV_FETCH_ASSOC) ) {
            ...
            // count npl
            $sql1 = "SELECT total_kelolaan FROM report_cabang WHERE kanwil = 'Kanwil 1' AND segmen = '$segmen' AND cabang_induk = '$cabang_induk'";

            $hasil1 = sqlsrv_query( $conn, $sql1);

            $s = 1;
            while( $row_k = sqlsrv_fetch_array( $hasil1, SQLSRV_FETCH_ASSOC) ) {
                $total_kelolaan_cabang = (int) $row_k['total_kelolaan'];

                $segmen2    = str_replace(' ', '_', $segmen);
                $segmen3     = str_replace('&', 'dan', $segmen2);

                $sql13 = "SELECT * FROM report_npl_kanwil1_cabang WHERE nama = '$cabang_induk'";

                $hasil13 = sqlsrv_query( $conn, $sql13);

                if (sqlsrv_has_rows( $hasil13 ) === true) {

                    $sql_input = "UPDATE report_npl_kanwil1_cabang SET nama = '$cabang_induk', urutan = 2, $segmen3 = $total_kelolaan_cabang WHERE nama = '$cabang_induk'";
                
                    $hasil_in = sqlsrv_query( $conn, $sql_input);
                    
                } else {

                    $sql_input = "INSERT INTO report_npl_kanwil1_cabang (nama, urutan, $segmen3) values('$cabang_induk', 2, $total_kelolaan_cabang)";
                ...
                //             $sql13c = "SELECT * FROM report_npl WHERE nama = '$cabang_induk'";

                //             $hasil13c = sqlsrv_query( $conn, $sql13c);

                //             if (sqlsrv_has_rows( $hasil13c ) === true) {

                //                 $sql_inputc = "UPDATE report_npl SET nama = '$cabang_induk', cabang_induk='$cabang_induk', urutan = 2, $segmen3c = $total_kelolaan_cabang WHERE nama = '$cabang_induk'";
                            
                //                 $hasil_inc = sqlsrv_query( $conn, $sql_inputc);
                                
                //             } else {

                //                 $sql_inputc = "INSERT INTO report_npl (nama, cabang_induk, urutan, $segmen3c) values('$cabang_induk', '$cabang_induk', 2, $total_kelolaan_cabang)";
                ...
                //             // ao
                //             $sql0ca = "SELECT DISTINCT c.reg_employee, c.name FROM tr_kelolaan a JOIN m_debitur b ON (a.deal_reff = b.deal_reff) JOIN m_employee c ON (a.reg_employee = c.reg_employee) WHERE b.cabang_induk = '$cabang_induk' AND b.kanwil = '$kanwil'";
                ...
                //                         $sql13ca = "SELECT * FROM report_npl WHERE nama = '$ao'";

                //                         $hasil13ca = sqlsrv_query( $conn, $sql13ca);

                //                         if (sqlsrv_has_rows( $hasil13ca ) === true) {

                //                             $sql_inputca = "UPDATE report_npl SET nama = '$ao',  cabang_induk='$cabang_induk', reg_employee = '$reg_employee', urutan = 3, $segmen3ca = $total_kelolaan_ao WHERE nama = '$ao'";
                ...
                //                             $sql_inputca = "INSERT INTO report_npl (nama, cabang_induk, reg_employee, urutan, $segmen3ca) values('$ao', '$cabang_induk',  '$reg_employee', 3, $total_kelolaan_ao)";
                ...
                //                                     $sql13cak = "SELECT * FROM report_npl WHERE nama = '$kolektibilitas' AND cabang_induk = '$cabang_induk' AND reg_employee = '$reg_employee'";

                //                                     $hasil13cak = sqlsrv_query( $conn, $sql13cak);

                //                                     if (sqlsrv_has_rows( $hasil13cak ) === true) {

                //                                         $sql_inputcak = "UPDATE report_npl SET nama = '$kolektibilitas', urutan = 4, $segmen3cak = $total_kelolaan_kolek, cabang_induk = '$cabang_induk', reg_employee = '$reg_employee' WHERE nama = '$kolektibilitas' AND cabang_induk = '$cabang_induk' AND reg_employee = '$reg_employee'";
                ...
                //                                         $sql_inputcak = "INSERT INTO report_npl (nama, urutan,$segmen3cak, cabang_induk, reg_employee) values('$kolektibilitas', 4, $total_kelolaan_kolek, '$cabang_induk', '$reg_employee')";
