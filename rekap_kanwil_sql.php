...
while( $row = sqlsrv_fetch_array( $hasil0, SQLSRV_FETCH_ASSOC) ) {
    ...
    // count npl
    $sql1 = "SELECT COUNT(b.deal_reff) as count_npl FROM tr_kelolaan a INNER JOIN m_debitur b ON ( a.deal_reff = b.deal_reff ) WHERE a.stat = 1 AND b.jenis_debitur= 'npl' AND b.kanwil = '$kanwil'";
    ...
    // sum npl
        $sql2 = "SELECT SUM(b.outstanding) as sum_npl FROM tr_kelolaan a INNER JOIN m_debitur b ON ( a.deal_reff = b.deal_reff ) WHERE a.stat = 1 AND b.jenis_debitur= 'npl' AND b.kanwil = '$kanwil'";
    ...
    // potensi_npl
        ...
        JOIN tr_kelolaan c ON ( b.deal_reff = c.deal_reff ) WHERE a.hasil_komitmen <> 'Failed' AND b.jenis_debitur = 'npl' AND b.kanwil = '$kanwil' AND c.stat = 1  AND MONTH ( a.tgl_komitmen ) = MONTH (CONVERT (DATE,GETDATE()))";
    ...
    // count wo
        $sql11 = "SELECT COUNT(b.deal_reff) as count_wo FROM tr_kelolaan a INNER JOIN m_debitur b ON ( a.deal_reff = b.deal_reff ) WHERE a.stat = 1 AND b.jenis_debitur= 'wo' AND b.kanwil = '$kanwil'";
    ...
    // sum wo
        $sql22 = "SELECT SUM(b.tunggakan_pokok) as sum_wo FROM tr_kelolaan a INNER JOIN m_debitur b ON ( a.deal_reff = b.deal_reff ) WHERE a.stat = 1 AND b.jenis_debitur= 'wo' AND b.kanwil = '$kanwil'";
    ...
    // monitoring_wo
        ...
        JOIN tr_kelolaan c ON ( b.deal_reff = c.deal_reff ) WHERE a.hasil_komitmen <> 'Failed' AND b.jenis_debitur = 'wo' AND b.kanwil = '$kanwil' AND c.stat = 1  AND MONTH ( a.tgl_komitmen ) = MONTH (CONVERT (DATE,GETDATE()))";
        ...
        $sql00 = "SELECT * FROM rekap_kanwil WHERE kanwil = '$kanwil'";

        $hasil00 = sqlsrv_query( $conn, $sql00);

        if (sqlsrv_has_rows( $hasil00 ) === true) {
            $sql111 = "UPDATE rekap_kanwil SET kanwil = '$kanwil', count_npl = $count_npl , sum_npl = $sum_npl, monitoring_npl = $monitoring_npl, potensi_npl = $potensi_npl, count_wo = $count_wo, sum_wo = $sum_wo , monitoring_wo = $monitoring_wo , potensi_wo = $potensi_wo WHERE kanwil = '$kanwil'";
        } else {
            $sql111 = "INSERT INTO rekap_kanwil (kanwil, count_npl , sum_npl, monitoring_npl, potensi_npl, count_wo, sum_wo , monitoring_wo , potensi_wo) values('$kanwil', $count_npl , $sum_npl, $monitoring_npl, $potensi_npl, $count_wo, $sum_wo , $monitoring_wo , $potensi_wo)";
        }
